...
Personal Dashboard ships four, all reading the recentchanges table: `recentchanges` (recent edits wiki-wide), `watchlist` (edits to pages the viewer watches), `recentlyedited` (other people's edits to pages the viewer worked on) and `mostedited` (edits to pages CirrusSearch relates to the ones the viewer edits most). They share [`ChangesListFeedSource`](../src/Feed/ChangesListFeedSource.php), which owns the query they have in common and keeps the two viewer-relative filters — hiding the viewer's own edits, and showing only unpatrolled edits — as options, so the same class can be registered again with personalization turned off.
