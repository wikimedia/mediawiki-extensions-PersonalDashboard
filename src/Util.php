...
class Util {
	...
	/**
	 * Log an error. Configuration errors are logged to the PersonalDashboard channel,
	...
	public static function logException(
		...
		if ( $exception instanceof WikiConfigException ) {
			LoggerFactory::getInstance( 'PersonalDashboard' )
				->log(
					$level,
	...
	/**
	 * Log a StatusValue object, either as a production error or in the PersonalDashboard channel,
	 * depending on its OK flag.
	 * @param StatusValue $status
	 */
	public static function logStatus( StatusValue $status ) {
		[ $errorText, $context ] = Status::wrap( $status )->getPsr3MessageAndContext();
		if ( $status->isOK() ) {
			LoggerFactory::getInstance( 'PersonalDashboard' )->error( $errorText, $context );
		} else {
	...
	/**
	 * Log a string in the PersonalDashboard channel.
	 * @param string $message
	 * @param array $context
	 */
	public static function logText( string $message, array $context = [] ) {
		LoggerFactory::getInstance( 'PersonalDashboard' )->error( $message, $context + [
	...
	public static function getJsonUrl(
		...
		$options = [
			'method' => 'GET',
			'userAgent' => $requestFactory->getUserAgent() . ' PersonalDashboard',
		];
		if ( $isSameFarm ) {
		...
		if ( !$warningStatus->isGood() ) {
			LoggerFactory::getInstance( 'PersonalDashboard' )->warning(
